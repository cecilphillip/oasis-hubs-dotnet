...
public class InitiateFundsTransferRequestHandler : RequestHandlerAsync<InitiateFundsTransferCommand> {
   ...
   [UseInboxAsync(step: 0, contextKey: typeof(InitiateFundsTransferRequestHandler), onceOnly: true,
   ...
   public override async Task<InitiateFundsTransferCommand> HandleAsync(
      ...
      foreach (var (accountId, bookings) in groupedBookings) {
         ...
         await this._stripeClient.V1.Transfers.CreateAsync(transOptions, cancellationToken: cancellationToken);
